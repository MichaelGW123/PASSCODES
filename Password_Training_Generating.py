...
############################## Part 0 Imports and Variable Assignment ################################################
...
check_to_load = "./training_checkpoints/{num_layers} Hidden Layers/ckpt_{specific_file}-{EPOCHS}"
...
checkpoint_path = f'./model_weights/{num_layers} Hidden Layers/{specific_file}/model({rnn_units})({EPOCHS})-{specific_file}'

if (save_new_weight):  # If we are saving a weight from the checkpoint, simply load, save, exit
    model.load_weights(
        f'./training_checkpoints/{num_layers} Hidden Layers/ckpt_{specific_file}-{EPOCHS}').expect_partial()
    model.save_weights(checkpoint_path)
    print(f"Saved weight from ckpt_{specific_file}-{EPOCHS}")
    exit()
