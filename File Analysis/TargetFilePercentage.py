...
with open(generated_file_path, 'r') as generated_file: # Loops through generated file
    for line in generated_file:
        ...
        if generated_length%50000000 == 0:
            print(f'{generated_length} completed') # To indicate progress and show program is properly running on larger datasets
        if current_line in unique_target_words: # When the current generated passwords matches a password in the target file
            found += 1
        ...
        array_percentages.append(percent) # Add whatever percentage currently matched to the array
end = time.time()
...
array_percentages = np.array(array_percentages) # will be the y axis in the graph
generated_list_length_array = np.arange(generated_length+1) # create the x axis in the graph
